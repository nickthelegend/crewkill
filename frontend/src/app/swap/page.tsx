...
import { SpaceBackground, AmongUsSprite } from "@/components/game";

// Floating sprite component for background decoration
const FloatingSprite = ({ colorId, delay = 0, x, y, size = 40 }: { colorId: number, delay?: number, x: string, y: string, size?: number }) => (
  <motion.div
    initial={{ x: 0, y: 0 }}
    animate={{ 
      y: [0, -40, 0, 40, 0],
    ...
    style={{ left: x, top: y }}
  >
    <AmongUsSprite colorId={colorId} size={size} />
  </motion.div>
);
  ...
  return (
    <SpaceBackground>
      <div className="min-h-[calc(100vh-80px)] w-full flex flex-col items-center justify-center px-4 relative overflow-hidden">
        {/* Decorative Floating Sprites */}
        <FloatingSprite colorId={0} x="10%" y="15%" size={60} delay={0} />
        <FloatingSprite colorId={1} x="85%" y="20%" size={40} delay={2} />
        <FloatingSprite colorId={2} x="15%" y="75%" size={50} delay={5} />
        <FloatingSprite colorId={3} x="80%" y="80%" size={45} delay={1} />
        <FloatingSprite colorId={4} x="45%" y="5%" size={35} delay={8} />

        {/* Background Glows (Enhanced for thematic depth) */}
            ...
            <div className="mt-4 px-2 space-y-3">
               <div className="flex justify-between items-center text-[10px] font-mono uppercase tracking-widest text-white/30">
                 <div className="flex items-center gap-1.5">
                   <span className="text-cyan-400 animate-pulse"><Info size={12} /></span>
