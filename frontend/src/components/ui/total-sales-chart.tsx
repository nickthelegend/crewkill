    ...
    <Card className={cn(
      ...
      <CardHeader className="flex flex-row items-center justify-between p-0">
        ...
        <Button
          variant="outline"
          size="sm"
          className="h-8 px-3 text-white/40 border-white/10 bg-transparent hover:bg-white/5 hover:text-white"
        >
          Details
        </Button>
      </CardHeader>
      ...
      <CardContent className="flex flex-col gap-4 p-0 pt-4">
        <div className="flex items-center gap-2 pb-2">
          ...
          <Badge
            variant="outline"
            className="rounded-full bg-emerald-500/10 text-xs text-emerald-400 border-emerald-500/20"
          >
            +2%
          </Badge>
        </div>

        <div className="group flex w-full -space-x-[1px] overflow-hidden rounded-xl border border-white/10">
          {periods.map((period) => (
            <button
              key={period.value}
              onClick={() => setSelectedPeriod(period.value)}
              data-active={selectedPeriod === period.value}
              className="relative flex h-8 flex-1 items-center justify-center bg-transparent text-[10px] font-black tracking-widest text-white/30 outline-none transition-colors hover:bg-white/5 data-[active=true]:bg-white/10 data-[active=true]:text-white border-r last:border-r-0 border-white/10"
            >
              {period.label}
            </button>
          ))}
